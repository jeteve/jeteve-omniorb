...
uv pip install -n --no-index --prerelease=allow --find-links=wheelhouse/ "jeteve_omniorb~=$OMNIORB_VERSION"
...
omkdepend > /dev/null
echo "✅ omkdepend works"

for binary in nameclt omniNames omniMapper; do
    rc=0
    $binary || rc=$?
